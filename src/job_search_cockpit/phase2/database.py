...
def create_phase2_engine(settings: Phase2Settings) -> Engine:
...
def phase2_session_factory_for(engine: Engine) -> sessionmaker[Session]:
    return sessionmaker(bind=engine, expire_on_commit=False)


def upgrade_phase2_database(database_url: str) -> None:
    ...
    configuration = Config(project_root / "alembic_phase2.ini")
    configuration.set_main_option("script_location", str(project_root / "alembic_phase2"))
    configuration.set_main_option("sqlalchemy.url", database_url)
    command.upgrade(configuration, "head")
    if database_url.startswith(sqlite_prefix):
        _protect_sqlite_files(database_path)
